...
class ApiService {
  final String baseUrl = 'http://192.168.102.17:3000'; // Your API URL  -  CHANGE THIS!

  Future<List<Expense>> getAllExpenses() async {
    try {
      final response = await http.get(Uri.parse('$baseUrl/expenses'));
      _debugPrint("getAllExpenses: URL: $baseUrl/expenses"); // Debug
      _debugPrint("getAllExpenses: Status Code: ${response.statusCode}"); // Debug
  ...
  Future<Expense> addExpense(Expense expense) async {
    try {
      ...
      _debugPrint("addExpense: Headers: ${<String, String>{'Content-Type': 'application/json; charset=UTF-8',}}");
      ...
      if (response.statusCode == 201) {
        ...
        throw Exception(responseData['message'] ?? 'Failed to add expense: ${response.statusCode}');
      }
    } catch (e) {
  ...
  Future<void> updateExpense(Expense expense) async {
    try {
      ...
      _debugPrint("updateExpense: Headers: ${<String, String>{'Content-Type': 'application/json; charset=UTF-8',}}");
      _debugPrint("updateExpense: Request Body: ${jsonEncode(expense.toMap())}");
      ...
      if (response.statusCode != 200) {
         final responseData = jsonDecode(response.body); // Attempt to decode
        throw Exception(responseData['message'] ?? 'Failed to update expense: ${response.statusCode}');
      }
    } catch (e) {
  ...
  Future<void> deleteExpense(Expense expense) async {
    try {
      ...
      _debugPrint("deleteExpense: Headers: ${<String, String>{'Content-Type': 'application/json; charset=UTF-8',}}");
      ...
      if (response.statusCode != 200) {
          final responseData = jsonDecode(response.body); // Attempt to decode
        throw Exception(responseData['message'] ?? 'Failed to delete expense: ${response.statusCode}');
      }
    } catch (e) {
